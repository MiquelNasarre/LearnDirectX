...
VSOut main(float3 pos : Position, float3 dylm0 : Dylm0a, float3 dylm1 : Dylm1a, float4 color : Color)
{
    VSOut vso;
    
    float3 dylm = tvalue.x * dylm1 + (1.f - tvalue.x) * dylm0;
    
    float3 norm = calculateNorm(dylm, pos);
    
    vso.norm = Q2V(qRot(quaternion, float4(0, norm)));
    vso.R3pos = Q2V(qRot(quaternion, float4(0, abs(dylm.x) * pos)));
    vso.color = color;
