...
float4 main(float4 pos : PointPos, float3 norm : Norm) : SV_Target
{
    Lightsource lights[4];
    lights[0].intensity = 60.f;
    lights[0].NEintensity = 0.f;
    lights[0].color = float4(1.f, 0.f, 0.f, 1.f);
    ...
    for (int i = 0; i < 4; i++)
    {
        float distance = normfloat3(lights[i].position - (float3) pos);
        float exposure = dotprd(norm, (lights[i].position - (float3) pos) / distance);
