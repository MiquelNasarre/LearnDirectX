...
Sphere::Sphere(Graphics& gfx, Color color, UINT depth)
{
	...
	auto pvs = (VertexShader*)AddBind(std::move(std::make_unique<VertexShader>(gfx, SHADERS_DIR + std::wstring(L"SphereVS.cso"))));

	AddBind(std::make_unique<PixelShader>(gfx, SHADERS_DIR + std::wstring(L"SpherePS.cso")));
