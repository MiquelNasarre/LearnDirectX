...
PointLight::PointLight(Graphics& gfx, Color color, Vector3f Position, float Radius, UINT circlePoints)
{
	...
	auto pvs = (VertexShader*)AddBind(std::move(std::make_unique<VertexShader>(gfx, L"Shaders/PointLightVS.cso")));
	AddBind(std::make_unique<PixelShader>(gfx, L"Shaders/PointLightPS.cso"));
